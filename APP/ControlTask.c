...
//	}	
		GMYPositionPID.ref = GimbalRef.yaw_angle_dynamic_ref;   //设定给定值
		GMYPositionPID.fdb = 0.20*GMYawEncoder.ecd_angle*GMYawRamp.Calc(&GMYawRamp);
//		static int count=0;
//		if(count>=200)
//		{
//			char temp[50];
//			sprintf(temp,"yawposref:%10.3f  ",GMYPositionPID.ref);	
//			UART3_PutStr (temp);//发送字符串
//			sprintf(temp,"yawposfdb:%10.3f  ",GMYPositionPID.fdb);	
//			UART3_PutStr (temp);//发送字符串
//			sprintf(temp,"yawangle:%10.3f  ",yaw_angle);	
//			UART3_PutStr (temp);//发送字符串
//			USART_SendData(USART3, '\n');
//			count=0;
//		}
...
//	GMPPositionPID.fdb = 0.5*GMPitchEncoder.ecd_angle* GMPitchRamp.Calc(&GMPitchRamp);    //加入斜坡函数
//	GMPPositionPID.Calc(&GMPPositionPID);   //得到pitch轴位置环输出控制量
//	//pitch speed control
//	GMPSpeedPID.ref = 0.1*GMPPositionPID.output;
//	GMPSpeedPID.fdb = MPU6050_Real_Data.Gyro_Y;
//	GMPSpeedPID.Calc(&GMPSpeedPID);
	
	GMPPositionPID.kp = 300;
	GMPPositionPID.kd = 1;
	
//	if(GimbalRef.pitch_angle_dynamic_ref>=3000)
//		GimbalRef.pitch_angle_dynamic_ref=3000;
//	if(GimbalRef.pitch_angle_dynamic_ref<=-2000)
//		GimbalRef.pitch_angle_dynamic_ref=-2000;
	
	error_last = error_now;
	error_now = GMPitchEncoder.ecd_angle-(-1*GimbalRef.pitch_angle_dynamic_ref-60);
	GMPSpeedPID.output = GMPPositionPID.kp*error_now+GMPPositionPID.kd*(error_now-error_last);
	
	if(GMPSpeedPID.output>=3000)
			GMPSpeedPID.output = 3000;
		if(GMPSpeedPID.output<=-3000)
			GMPSpeedPID.output = -3000;
...
//	{
//		char temp[50];
////		sprintf(temp,"pitposref:%10.3f  ",GMPPositionPID.ref);	
////		UART3_PutStr (temp);//发送字符串
////		sprintf(temp,"pitposfdb:%10.3f  ",GMPPositionPID.fdb);	
////		UART3_PutStr (temp);//发送字符串
////		sprintf(temp,"pitsperef:%10.3f  ",GMPSpeedPID.ref);	
////		UART3_PutStr (temp);//发送字符串
////		sprintf(temp,"pitspefdb:%10.3f  ",GMPSpeedPID.fdb);	
////		UART3_PutStr (temp);//发送字符串
////		sprintf(temp,"pitoutput:%10.3f  ",GMPSpeedPID.output);	
////		UART3_PutStr (temp);//发送字符串
////		USART_SendData(USART3, '\n');
//		sprintf(temp,"error_now:%10.3f  ",error_now);	
//		UART3_PutStr (temp);//发送字符串
//		sprintf(temp,"error_last:%10.3f  ",error_last);	
//		UART3_PutStr (temp);//发送字符串
//		sprintf(temp,"ecd_angle:%10.3f  ",GMPitchEncoder.ecd_angle);	
//		UART3_PutStr (temp);//发送字符串
//		sprintf(temp,"output:%10.3f  ",GMPSpeedPID.output);	
//		UART3_PutStr (temp);//发送字符串
//		count=0;
//	}
//	count++;
...
//	GMYSpeedPID.Calc(&GMYSpeedPID);		
		...
		GMYPositionPID.kd = 40;
	
		if(GimbalRef.yaw_angle_dynamic_ref>=200)
			GimbalRef.yaw_angle_dynamic_ref=200;
		if(GimbalRef.yaw_angle_dynamic_ref<=-200)
			GimbalRef.yaw_angle_dynamic_ref=-200;
	
		error_last_yaw = error_now_yaw;
		error_now_yaw = GMYawEncoder.ecd_angle+10*GimbalRef.yaw_angle_dynamic_ref-35;//改大
		GMYSpeedPID.output = GMYPositionPID.kp*error_now_yaw+GMYPositionPID.kd*(error_now_yaw-error_last_yaw);
		
		if(GMYSpeedPID.output>=2000)
			GMYSpeedPID.output = 2000;
		if(GMYSpeedPID.output<=-2000)
			GMYSpeedPID.output = -2000;
...
//	{
//		char temp[50];
//		sprintf(temp,"error_now:%10.3f  ",error_now_yaw);	
//		UART3_PutStr (temp);//发送字符串
//		sprintf(temp,"error_last:%10.3f  ",error_last_yaw);	
//		UART3_PutStr (temp);//发送字符串
//		sprintf(temp,"ecd_angle:%10.3f  ",GMYawEncoder.ecd_angle);	
//		UART3_PutStr (temp);//发送字符串
//		sprintf(temp,"output:%10.3f  ",GMYSpeedPID.output);	
//		UART3_PutStr (temp);//发送字符串
//		count=0;
//	}
//	count++;
...
//  {
//		char temp[50];
//		sprintf(temp,"yawsperef:%10.3f  ",GMYSpeedPID.ref);	
//		UART3_PutStr (temp);//发送字符串
//		sprintf(temp,"yawspefdb:%10.3f  ",GMYSpeedPID.fdb);	
//		UART3_PutStr (temp);//发送字符串
//		count=0;
//  }		
//	count++;
...
{
	//云台控制输出								
//	if((GetWorkState() == STOP_STATE) || GetWorkState() == CALI_STATE)//if((GetWorkState() == STOP_STATE) ||Is_Serious_Error() || GetWorkState() == CALI_STATE)   
//	{
...
//	}		
	  //Set_Gimbal_Current(CAN2, 0, (int16_t)GMPSpeedPID.output);     //yaw + pitch	
	  ...
	  {
			Set_Gimbal_Current(CAN2, 0, GMPSpeedPID.output);     //yaw + pitch	
		}
