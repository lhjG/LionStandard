...
void MouseShootControl(Mouse *mouse)
{
	...
	switch(friction_wheel_state)
	{
		...
		case FRICTION_WHEEL_START_TURNNING:
		{
			...
			if(closeDelayCount>50 || count%2 == 0)   //关闭摩擦轮
			{
				LASER_OFF();
				friction_wheel_state = FRICTION_WHEEL_OFF;				  
				SetFrictionWheelSpeed(1000); 
...
void RemoteControlProcess(Remote *rc)
{
    ...
    {
        ChassisSpeedRef.forward_back_ref = 2.0*(RC_CtrlData.rc.ch1 - (int16_t)REMOTE_CONTROLLER_STICK_OFFSET) * STICK_TO_CHASSIS_SPEED_REF_FACT;
...
{
	VAL_LIMIT(GimbalRef.pitch_angle_dynamic_ref, -PITCH_MAX+10, 2*PITCH_MAX-20); //PITCH_MAX);
